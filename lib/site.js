...
const Asset = require('./parsers/asset')
const Post = require('./parsers/post')
const Page = require('./parsers/page')
const Data = require('./parsers/data')
const util = require('./util')

const writeAsset = require('./writers/asset')
const writeTemplated = require('./writers/templated')
...
function Site(attrs) {
  ...
  this.config.forEach(function(config) {
    if (fs.existsSync(config)) {
      ...
      Object.assign(site, yaml.load(content))
    }
  })
...
Site.prototype.parse = function() {
  ...
  if (!this.includeFuture) {
    this.posts = this.posts.filter(post => !post.future)
  }
  ...
  this.posts.forEach(function(post) {
    post.categories.forEach(function(cat) {
      var category = site.categories.filter(item => item[0] === cat)[0]
